...
async function getPublicVisitCount(env) {
  try {
    const totalRow = await env.MEGA_DB.prepare(`SELECT COUNT(*) as c FROM visits`).first();
    return json({ total: totalRow?.c || 0 });
  } catch (e) {
    // Table not migrated yet, or a D1 hiccup — badge just stays hidden client-side.
    return json({ total: 0 });
  }
}
...
async function fawaterkWebhook(env, request) {
  ...
  try {
    ...
    if (computed !== hashKey) return err("Signature verification failed", 401);
...
const LOGO_WATERMARK_WIDTH_PX = 110; // final width of the logo on the delivered photo, in pixels
const LOGO_WATERMARK_MARGIN_PX = 18; // gap from the bottom and right edges, in pixels
...
async function applyLogoWatermark(env, sourceImageUrl, request) {
  ...
  const composited = await env.IMAGES.input(sourceRes.body)
    .draw(env.IMAGES.input(logoRes.body).transform({ width: LOGO_WATERMARK_WIDTH_PX }), {
      bottom: LOGO_WATERMARK_MARGIN_PX,
      right: LOGO_WATERMARK_MARGIN_PX,
    })
