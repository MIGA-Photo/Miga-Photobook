...
async function createOrder(env, request) {
  ...
  const { productId, productTitle, price, phone, appUsed, ref, buyerName, buyerEmail, orderType } = body;
  ...
  const order = {
    code,
    // 'transform' (default, buys one AI photo transformation) or 'prompt'
    // (buys the text of the professional prompt behind that photo).
    ...
    // Filled in only when the buyer is logged into a customer account at
    // checkout — guest checkouts simply leave these blank, never invented.
    ...
    transformUsed: false,
    resultUrl: null,
    // For orderType 'prompt': snapshotted from the product at approval time,
    // never before — see approveOrder(). Only ever exposed via /orders/track
    // once status is 'approved', i.e. only to whoever holds this order's code.
    promptText: null,
  };
  await env.MEGA_KV.put(`order:${code}`, JSON.stringify(order));
...
async function trackOrder(env, request) {
  ...
  const order = JSON.parse(raw);
  // The transformation instructions never leave the server for a 'transform'
  // order, not even here — this site transforms the photo for the customer
  // automatically. The order code itself (which the browser already holds)
  // is what /transform later accepts as proof of purchase.
  //
  // A 'prompt' order is different by design: the prompt text IS the product
  // being sold, so once approved (payment confirmed), it's returned here —
  // but only ever to whoever already holds this specific order code.
...
// Public, no-password endpoint — deliberately returns only {productId: count}
// (approved-order counts), never order details, phone numbers, names, or
// anything else from the orders themselves. Powers the storefront's "الأكثر
// طلبًا" (Most Requested) section and the admin product list's per-item count.
...
async function approveOrder(env, request) {
  ...
  order.status = "approved";
  if (order.orderType === "prompt" && !order.promptText) {
    // Snapshot the prompt text at the moment of approval — this is what the
    // customer actually paid for, so it stays fixed even if the product's
    // prompt gets edited by the admin later.
    const productRaw = await env.MEGA_KV.get(`product:${order.productId}`);
    if (productRaw) {
      const product = JSON.parse(productRaw);
      order.promptText = product.prompt || null;
...
// Marks a pending order as rejected — used for invalid or fake transfer claims.
// Kept in the list (not deleted) so there's still a record of it, but it no
// longer counts as pending and can never be approved or used for a transform.
...
// Permanently removes a rejected order from the list — for cleaning up
// confirmed spam/fake entries. Only ever allowed on already-rejected orders,
// never on pending or approved ones, so this can't be used to hide a real
// transaction.
...
async function paymobWebhook(env, request) {
  ...
  try {
    ...
    // Was previously "if hmacFromPaymob is present AND wrong, reject" — which
    // meant a request with NO hmac param at all silently skipped verification
    // entirely and could approve any order for free. Now the hmac is always
    // required; missing it is treated the same as an invalid one.
...
async function fawryWebhook(env, request) {
  ...
  // This previously trusted `orderStatus`/`merchantRefNumber` with NO signature
  // check at all — anyone who found this URL could POST a fake "PAID" callback
  // for any order code and get it approved for free. Now it verifies
  // `messageSignature` before trusting anything in the payload, and fails
  // closed (rejects) if the secret isn't configured or the signature is
  // missing/wrong.
  ...
  try {
    ...
    // ⚠️ Field concatenation order per Fawry's Server Notification V2 docs —
    // verify this against a real test callback in your Fawry merchant
    // dashboard before relying on it in production; Fawry's exact field
    // order/casing can vary by integration.
...
async function transformImage(env, request) {
  ...
  try {
    const falBody = {
      ...
      image_urls: [image],
      // Highest resolution the model supports — $0.16/image on fal.ai vs
      // $0.12 at 2K (per fal.ai's published pricing for nano-banana-2/edit).
      resolution: "4K",
      output_format: "png",
    };
